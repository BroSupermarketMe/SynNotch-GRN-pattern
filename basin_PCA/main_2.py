...
def main(N):
    # 1. 读取.mat文件
    mat_path = f"C:\\Users\\JiajunTang\\Desktop\\20231106代码\\Lateral_inhibition-master\\new_figures_random_growth_basin\\cell_generation_{N}\\Hs_1000.mat"
    ...
    CV_path = f"C:\\Users\\JiajunTang\\Desktop\\20231106代码\\Lateral_inhibition-master\\new_figures_random_growth\\cell_generation_{N}CVs_pedigree_100.mat"
    CV_mat = scipy.io.loadmat(CV_path)
    # 搞反了 slow其实是fast
    CV_slow_path = f"C:\\Users\\JiajunTang\\Desktop\\20231106代码\\Lateral_inhibition-master\\new_figures_random_growth\\cell_generation_slow_{N}CVs_pedigree_100.mat"
    CV_slow_mat = scipy.io.loadmat(CV_slow_path)


    CV_G_i = CV_mat['CVs_Gal4']
    CV_F_i = CV_mat['CVs_surEGFP']

    CV_G_i_slow = CV_slow_mat['CVs_Gal4']
    CV_F_i_slow = CV_slow_mat['CVs_surEGFP']

    G_freqs, G_bins = freq_norm(CV_G)
    F_freqs, F_bins = freq_norm(CV_F)
    G_i_freqs, G_i_bins = freq_norm(CV_G_i)
    F_i_freqs, F_i_bins = freq_norm(CV_F_i)
    G_i_slow_freqs, G_i_slow_bins = freq_norm(CV_G_i_slow)
    F_i_slow_freqs, F_i_slow_bins = freq_norm(CV_F_i_slow)

    plt.figure(figsize=(12, 8))
    plt.bar(G_bins[:-1], G_freqs, width=np.diff(G_bins), align='edge', label='Gal4', color='red', alpha=0.5)
    plt.bar(G_i_bins[:-1], G_i_freqs, width=np.diff(G_i_bins), align='edge', label='Gal4_slow', color='sienna', alpha=0.5)
    plt.bar(G_i_slow_bins[:-1], G_i_slow_freqs, width=np.diff(G_i_slow_bins), align='edge', label='Gal4_fast', color='orange', alpha=0.5)
    plt.bar(F_bins[:-1], F_freqs, width=np.diff(F_bins), align='edge', label='surEGFP', color='green', alpha=0.5)
    plt.bar(F_i_bins[:-1], F_i_freqs, width=np.diff(F_i_bins), align='edge', label='surEGFP_slow', color='c', alpha=0.5)
    plt.bar(F_i_slow_bins[:-1], F_i_slow_freqs, width=np.diff(F_i_slow_bins), align='edge', label='surEGFP_fast', color='yellow', alpha=0.5)
    plt.xlabel('CV')
    plt.ylabel('Normalized Frequency')
    plt.legend()
    plt.savefig(f"C:\\Users\\JiajunTang\\Desktop\\20231106代码\\Lateral_inhibition-master\\new_figures_random_growth_basin\\cell_generation_{N}\\CV.png")

    # 取平均值
    CV_G = np.mean(CV_G)
    CV_F = np.mean(CV_F)
    CV_G_i = np.mean(CV_G_i)
    CV_F_i = np.mean(CV_F_i)
    CV_G_slow_i = np.mean(CV_G_i_slow)
    CV_F_slow_i = np.mean(CV_F_i_slow)
    return CV_G, CV_F, CV_G_i, CV_F_i, CV_G_slow_i, CV_F_slow_i

CVs_G, CVs_F, CVs_G_i, CVs_F_i, CVs_G_slow_i, CVs_F_slow_i = [], [], [], [], [], []
for i in range(8, 21, 2 ):
    CV_G, CV_F, CV_G_i, CV_F_i, CV_G_slow_i, CV_F_slow_i = main(i)
    CVs_G.append(CV_G)
    CVs_F.append(CV_F)
    CVs_G_i.append(CV_G_i)
    CVs_F_i.append(CV_F_i)
    CVs_G_slow_i.append(CV_G_slow_i)
    CVs_F_slow_i.append(CV_F_slow_i)
# 绘制CV的变化
plt.figure(figsize=(16, 8))
plt.subplot(1,2,1)
plt.plot(range(8, 21, 2), CVs_G, label='Gal4', color='red', marker='o')
plt.plot(range(8, 21, 2), CVs_G_slow_i, label='Gal4_fast', color='orange', marker='o')
plt.plot(range(8, 21, 2), CVs_G_i, label='Gal4_slow', color='sienna', marker='o')
plt.legend()
plt.xlabel('Cell Generation')
plt.ylabel('CV')
plt.subplot(1,2,2)
plt.plot(range(8, 21, 2), CVs_F, label='surEGFP', color='green', marker='o')
plt.plot(range(8, 21, 2), CVs_F_slow_i, label='surEGFP_fast', color='yellow', marker='o')
plt.plot(range(8, 21, 2), CVs_F_i, label='surEGFP_slow', color='c', marker='o')
plt.legend()
plt.xlabel('Cell Generation')
plt.ylabel('CV')
plt.savefig(f"C:\\Users\\JiajunTang\\Desktop\\20231106代码\\Lateral_inhibition-master\\new_figures_random_growth_basin\\CVs.png")